        ...
        if(tableExists(tableName)){
            try {
             table = conf.getHconnection().getTable(TableName.valueOf(tableName));
                System.out.println("表存在，且获取表连接成功"+table);
            } catch (IOException e) {
                System.out.println("获取表连接失败"+e);
                LOG.error(null,e);
            }
        }
        return table;
...
/*
        ...
        HBaseTableUtil.deleteTable("ns1:t1");
        HBaseTableUtil.deleteTable("sst");
        HBaseTableUtil.deleteTable("test:aaaa");
