 ...
 * @Created by robot
 **/
object HiveConfigPar extends Serializable with Logging {
  ...
  var hiveParSql: java.util.HashMap[String, String] = null //存放表名与SQL之间的映射关系
  ...
  def init(): Unit = {
    ...
    println("====================初始化tables===[hivePartitionTable]================")
    hiveParSql = getHiveParSQL
    hiveParSql.foreach(table => {
      println(table)
    })
  ...
  def getHiveParSQL(): java.util.HashMap[String, String] = {
